 ...
 *
 * The calculation itself lives in media/cartShared.js because the webview
 * needs it too -- it shows a live Tm for the current selection -- and one
 * implementation is the only way the two can be guaranteed to agree. This
 * module adds the host-only policy on top: prefer a Tm the file already
 * states over one we compute.
 */
'use strict';

const { tmNebQ5 } = require('../media/cartShared');
